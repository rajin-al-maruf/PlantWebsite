...
const comboFilterInfo = [
  { id: "category", title: "Combo Type", options: ["Executive Duo", "Executive Trio", "Student Combo", "Desk Combo"] },
...
const ComboPage = ({plants, setPlants}: ComboPageProps) => {
  ...
  const [filter, setFilter] = useState<ComboFilterState>({ category: [], availability: [], priceRange: [] })
  const [sortBy, setSortBy] = useState("newest")
  const [tempFilter, setTempFilter] = useState<ComboFilterState>(filter)
  const [showFilters, setShowFilters] = useState(true)

  const totalPages = Math.ceil(totalCount / ITEMS_PER_PAGE);
  
  const handleSetFilter: Dispatch<SetStateAction<ComboFilterState>> = (value) => {
    setFilter(value);
    setPage(1);
  };
  ...
  useEffect(() => {
    const fetchPlants = async () => {
      try {
        setIsLoading(true)
        let plantData = supabase.from('plants').select('*', { count: 'exact' }).eq('product_type', 'Combo')
  ...
  return (
    <div className='max-w-6xl 2xl:max-w-7xl mx-auto mt-36 px-4 md:px-6 lg:px-8 xl:px-0'>
      ...
      <div className='relative w-full h-64 md:h-80 my-8 rounded-[2.5rem] overflow-hidden group'>
        <img src='/assets/bg-web.jpg' alt="Shop Our Combos" className='absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105' />
        ...
        <div className="absolute inset-0 p-8 md:p-12 lg:p-16 flex flex-col justify-center text-white">
            ...
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-4">Plant Combos</h1>
            <p className="max-w-md text-sm md:text-base text-gray-200 leading-relaxed">
               Perfectly paired plants to instantly transform your space. Save more when you buy them together.
            </p>
        </div>
      </div>
      ...
      <div className='md:grid grid-cols-4 gap-10 mt-6'>
        <div className="hidden md:block">
          {comboFilterInfo.map((info, index) => (
        ...
        {isLoading ? (
          ...
          <div className='w-full mt-20 flex flex-col items-center justify-center text-neutral-400 col-span-1 md:col-span-3 gap-4'>
             ...
             <h2 className="text-2xl font-semibold text-brand-primary-dark">No combos found</h2>
